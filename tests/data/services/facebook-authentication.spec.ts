...
describe('FacebookAuthenticationService', () => {
  ...
  it('should call CreateUserByEmailRepo when LoadFacebookUserApi returns undefined', async () => {
    loadUserAccountRepo.load.mockResolvedValueOnce(undefined)

    await sut.perform({ token })

    expect(createFacebookAccountRepo.createFacebookAccount).toHaveBeenCalledWith({
      name: 'any_fb_name',
      email: 'any_fb_email',
      facebookId: 'any_fb_id'
    })

    expect(createFacebookAccountRepo.createFacebookAccount).toHaveBeenCalledTimes(1)
  })
})
